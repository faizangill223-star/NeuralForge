...
export default function WhatsAppButton() {
  ...
  const whatsappNumber = settings.whatsapp_number || '923000000000';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex items-center gap-3 group">
      {/* Tooltip label */}
      <div
        className={`glass-strong px-4 py-2.5 rounded-xl shadow-lg transition-all duration-300 ${
          showLabel ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4 pointer-events-none'
        }`}
      >
        <p className="text-sm font-medium text-white whitespace-nowrap">Need help? Chat with us</p>
        <p className="text-xs text-neutral-400">We typically reply in minutes</p>
      </div>
